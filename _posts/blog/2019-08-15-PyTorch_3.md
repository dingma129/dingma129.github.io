...
In this blog, I will introduce 2 different methods to create a convolutional neural network (CNN) using `PyTorch`, by using `torch.nn.Sequential` (similar to Keras Sequential Model) and by inheriting from `torch.nn.Module`(similar to Keras functional API).
...
* In `nn.Sequential`, the `nn.Module`'s stored inside are connected in a cascaded way. It's quite similar to `keras.models.Sequential`;
* `nn.Module`is quite similar to keras' functional API `keras.models.Model`, and the structure of layers can be other than linear.
